...
async def _require_admin(message: Message, bot: Bot) -> bool:
    ...
    if not await is_admin(bot, message.chat.id, message.from_user.id):
        err = await message.answer("❌ Только администраторы могут использовать эту команду.")
...
async def cmd_setdelay(message: Message, bot: Bot) -> None:
    ...
    if len(parts) != 2 or not parts[1].lstrip("-").isdigit():
        await smart_reply(
            ...
            "Диапазон: 0 – 86 400 с. (0 = мгновенное удаление)",
        )
        return

    delay = int(parts[1])
    if not (0 <= delay <= 86_400):
        await smart_reply(message, bot, "❌ Задержка должна быть от 0 до 86 400 секунд.")
        return
